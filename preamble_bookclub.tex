% \documentclass[a4paper,table,twocolumn]{report}
\usepackage{etex}
...
% \setmathfont[Scale=MatchUppercase]{STIX2Math.otf}
% \setmathfont[Scale=MatchUppercase]{LibertinusMath-Regular}
...
\fancypagestyle{bookclub}{\fancyhf{}
    % \fancyfootoffset{10mm}
     % \rfoot[\textcolor{Col1}{Patra's Book Club}]{\tikz {\node[fill=Col1,text=white]{\bfseries\thepage};}}
  \rfoot[]{\tikz[baseline=-3pt]{\node[fill=Col1,text=white]{\bfseries\thepage};}}
  % \rfoot[]{\tikz[baseline=0pt]{\node[fill=Col1,text=white] (page) {\bfseries\thepage};
  %      \node[xshift=-3mm,anchor=east] (chapt) {\textcolor{Col1}{\leftmark{}}};
  %  }}
  % \rfoot[]{\textcolor{Col1}{\thepage}}
      % \node[Col1,anchor=east,xshift=-5pt] (text) at (page) {\leftmark{}};
    % \fill[Col1] (text2.east) rectangle++(5mm,5mm);
  % }
    % \fill[Col1] ([shift={(0mm,-2mm)}]text.east) rectangle++(5mm,5mm);
    % \rfoot[\tikz {\node[fill=Col1,text=white]{\bfseries\thepage};
     % \cfoot[]{\textcolor{Col2}{Η Μεγάλη Χίμαιρα}}
    \lfoot[]{\textcolor{Col1}{Patra's Book Club}}
    % \lhead[\textcolor{Col1}{Η Μεγάλη Χίμαιρα}]{}
    \rhead[]{\tikz[remember picture,overlay]{\node[rotate=90,anchor=east] (text) at
        ([shift={(-5mm,-8mm)}]current page.north east) {\textcolor{Col2}\leftmark{}};
    \fill[Col2] ([xshift={-2.5mm}]text.east) rectangle++(5mm,5mm);
}}
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%% mycomments - myquotes %%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newsavebox{\boxs} %define the name of the new savebox
\newenvironment{com}[1]{
%grab the typesetting argument in {begin def}, where you can typeset it, by using 
% the \savebox command.
% and then use it (move it, typeset it) with  the \usebox command in the {end def}
\savebox{\boxs{#1}}\begin{quote}\small\itshape}
{\newline\hspace*\fill\usebox{\boxs}\end{quote}}
